...
public:
    ...
    {}
    //mv ctor
    RTTIAnyType(RTTIAnyType&& Other) noexcept
        :mlb_type(Other.mlb_type),ReallyPtr(Other.copy())
    {}
    ...
    template<typename U , typename = std::enable_if_t<!std::is_same_v<std::decay_t<U>,RTTIAnyType>>>
    RTTIAnyType(U&& Value)
        :
        ...
        //mlb_type(std::type_index(typeid(typename std::decay<U>::type)))
    {
        ...
        //
        static_assert(!std::is_same<typename std::decay<U>::type,const char*>::value,"Don't Use const char* as Param . Use FString");
    }
